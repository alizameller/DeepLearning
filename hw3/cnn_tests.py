...
def test_output_shape():
    ...
    input_shape = tf.cast(
        rng.normal(shape=[batch_size, 28, 28, input_depth]), tf.float32
    )
    ...
    classifier = Classifier(
        input_height, input_width, input_depth, layers, layer_kernel_sizes, n_classes
    )
    tf.debugging.assert_equal(
        classifier(input_shape).shape, (batch_size, n_classes), summarize=2
    )
...
def test_adam():
    ...
    old_variable_sum = tf.reduce_sum(
        [tf.reduce_sum(i) for i in classifier.trainable_variables]
    )
