...
def loadData(filename):   
    ...
    images = image_data.reshape(len(image_data),3,32,32).transpose(0,2,3,1)
    labels = data_batch['fine_labels']
...
if __name__ == "__main__":
    ...
    # that directory
    train_images, train_labels = loadData('train')

    meta_file = r'./cifar-100-python/meta'
    meta_data = unpickle(meta_file)
    label_names = meta_data['fine_label_names']

    test_images, test_labels = loadData('test')

    '''# dispaly random nine images
    # define row and column of figure
    rows, columns = 3, 3
    # take random image idex id
    imageId = np.random.randint(0, len(test_images), rows * columns)
    # take images for above random image ids
    test_images = test_images[imageId]
    # take fine labels for these images only
    test_labels = [test_labels[j] for j in imageId]
    # define figure
    ...
    for i in range(1, columns*rows +1):
        ...
        plt.yticks([])
        plt.title("{}"
            .format(label_names[test_labels[i-1]]))
    plt.show()
    exit()'''
    ...
    resnet = ResNet(3, layer_depths, 100)
    ...
    print(top_k_accuracy_score(test_labels, test_labels_hat, k=5))
