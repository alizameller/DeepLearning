...
class BasisExpansion(tf.Module):
    ...
    def __call__(self, x):
        gaussian = tf.math.exp(-tf.square(x[:, tf.newaxis] - self.mu)/(tf.square(self.sigma)))
...
if __name__ == "__main__":
    ...
    x = rng.uniform(shape=(num_samples, num_inputs), minval=lower_bound, maxval=upper_bound)
    ...
    for i in bar:
        ...
        grads = tape.gradient(loss, linear.trainable_variables + basis_expansion.trainable_variables)
        grad_update(step_size, linear.trainable_variables + basis_expansion.trainable_variables, grads)
    ...
    ax[0].plot(a.numpy().squeeze(), linear(basis_expansion(a)).numpy().squeeze(), "r--", label='linear regression model')
    ...
    ax[1].plot(a2.numpy().squeeze(), np.array(basis_expansion(a2).numpy().squeeze()), "-", \
        label=['Basis Fn 1', 'Basis Fn 2', 'Basis Fn 3', 'Basis Fn 4', 'Basis Fn 5', 'Basis Fn 6', 'Basis Fn 7', 'Basis Fn 8'])
    ax[1].set_xlabel("x")
    ax[1].set_ylabel("y")
